...
public class HomeActivity extends SsrActivity {
    ...
    protected void changeState(BaseService.State state, @Nullable String msg, Boolean animate) {
        Log.e("zkq", "--> changeState()  state=" + state + ", msg=" + msg + ", animate=" + animate);
        ...
        if (state == BaseService.State.Connected) {
            ...
            tester.getStatus().observe(this, status -> {
                Log.e("zkq", "--> HttpTester test status=" + status);
                status.retrieve(text -> {
                    Log.e("zkq", "--> HttpTester test status.text=" + text);
                    return null;
                }, errMsg -> {
                    Log.e("zkq", "--> HttpTester test status.errMsg=" + errMsg);
                    return null;
                });
            });
        } else {
